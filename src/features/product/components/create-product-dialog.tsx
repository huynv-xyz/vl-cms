...
export function CreateProductDialog({ open, onOpenChange }: any) {
    ...
    const lastWarehouseIdRef = useRef<number | undefined>(undefined)

    useEffect(() => {
        if (open) {
            lastGroupIdRef.current = undefined
            lastWarehouseIdRef.current = undefined
        }
    }, [open])
    ...
    return (
        <CrudFormDialog<ProductFormValues, CreateProductRequest, unknown>
            ...
            mapFormToRequest={(v) => {
                const unit = resolveRequestUnit(v)
                return {
                    code: v.code,
                    name: v.name,
...
function resolveRequestUnit(values: ProductFormValues) {
