    ...
    return <PageSection title="Chứng từ kho" description="Theo dõi số lượng nhập xuất; không bao gồm giá trị." data={data} isLoading={isLoading} error={error}>
        {(result) => <div className="space-y-4">
            <Card className="gap-0 overflow-hidden border-border/60 py-0 shadow-sm"><CardHeader className="border-b bg-muted/40 px-4 py-3"><div className="flex flex-wrap items-center gap-2">
                ...
                <DateFilterInput aria-label="Từ ngày" className="h-10 min-w-[160px] flex-1 rounded-md border-slate-300 bg-white shadow-xs" value={search.from || ""} max={search.to} onChange={(value) => setFilter({ from: value })} />
                <DateFilterInput aria-label="Đến ngày" className="h-10 min-w-[160px] flex-1 rounded-md border-slate-300 bg-white shadow-xs" value={search.to || ""} min={search.from} onChange={(value) => setFilter({ to: value })} />
