...
const COLUMNS: ExportColumn[] = [
    ...
    { label: "Doanh thu trước chiết khấu", value: (row) => Number(row.sale_qty || 0) > 0 ? Number(row.sale_qty || 0) * Number(row.unit_price || 0) : 0, width: 24, type: "number" },
    { label: "Chiết khấu", value: (row) => row.discount, width: 18, type: "number" },
    { label: "Doanh thu sau chiết khấu (chưa VAT)", value: (row) => saleRevenue(row), width: 30, type: "number" },
    { label: "VAT", value: (row) => row.vat_code, width: 12 },
    { label: "Tiền VAT", value: (row) => row.vat_amount, width: 18, type: "number" },
    { label: "Giá trị trả lại sau chiết khấu (chưa VAT)", value: (row) => returnRevenue(row), width: 32, type: "number" },
    { label: "Doanh thu thuần sau chiết khấu (chưa VAT)", value: (row) => saleRevenue(row) - returnRevenue(row), width: 34, type: "number" },
    { label: "Doanh thu thuần sau chiết khấu (có VAT)", value: (row) => saleRevenue(row) - returnRevenue(row)
        + (Number(row.sale_qty || 0) > 0 ? Number(row.vat_amount || 0) : 0)
        - (Number(row.return_qty || 0) > 0 ? Number(row.vat_amount || 0) : 0), width: 34, type: "number" },
